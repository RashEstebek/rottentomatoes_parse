...
for i in range(1, 5):
    ...
    raw = soup.find_all('div', class_='row countdown-item')
    for data in raw:
        allFilms.append(
            {
                ...
                'year': int(data.find("span", class_="subtle start-year").text[1:-1]),
                'score': int(data.find("span", class_="tMeterScore").text.strip("%")),
